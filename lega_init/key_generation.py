...
class SecurityConfigGenerator:
    ...
    def generate_root_certs(self, country, country_code, location, org, email, org_unit, common_name, password):
        ...
        cert = (
            ...
            .add_extension(extval=x509.BasicConstraints(ca=True, path_length=0), critical=True)
            .add_extension(extval=x509.SubjectKeyIdentifier.from_public_key(key.public_key()), critical=False)
            .add_extension(extval=x509.AuthorityKeyIdentifier.from_issuer_public_key(key.public_key()), critical=False)
    ...
    def generate_csr(
        ...
        # Generate a CSR
        csr = (
            x509.CertificateSigningRequestBuilder().subject_name(subject).sign(key, hashes.SHA256(), default_backend())
        )
    ...
    def sign_certificate_request(self, service, password, custom_ca, java_services=None):
        ...
        if custom_ca and len(custom_ca) == 2:
            ...
            with custom_key.open(mode="rb") as root_key:
                root_ca_pkey = serialization.load_pem_private_key(
                    root_key.read(), password=password.encode("utf-8"), backend=default_backend()
                )
        else:
        ...
        cert = (
            ...
            .add_extension(
                extval=x509.AuthorityKeyIdentifier.from_issuer_public_key(root_ca_pkey.public_key()), critical=False
            )
            .sign(private_key=root_ca_pkey, algorithm=hashes.SHA256(), backend=default_backend())
        )
    ...
    def sign_certificate_request_dns(
        self, service, service_dns, password, custom_ca, java_services=None, kube_ns="default"
    ):
        """Sign Certificate Request based on root Certificate Authority (CA)."""
        with open(self._config_path / f"csr/{service}.csr.pem", "rb") as f:
            csr = x509.load_pem_x509_csr(data=f.read(), backend=default_backend())
